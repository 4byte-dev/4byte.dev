...
class UpdateReactCounts extends Command
{
    ...
    protected function updateCountsForModel(Model $model, ReactService $reactService, string $modelClass): void
    {
        if ($modelClass === Article::class) {
            $likesCount = Like::where('likeable_type', $modelClass)
                ->where('likeable_id', $model->getKey())
                ->count();
            $reactService->setCount($modelClass, $model->getKey(), 'likes', $likesCount);

            $dislikesCount = Dislike::where('dislikeable_type', $modelClass)
                ->where('dislikeable_id', $model->getKey())
                ->count();
            $reactService->setCount($modelClass, $model->getKey(), 'dislikes', $dislikesCount);

            $commentsCount = Comment::where('commentable_type', $modelClass)
                ->where('commentable_id', $model->getKey())
                ->count();
            $reactService->setCount($modelClass, $model->getKey(), 'comments', $commentsCount);
        }
        ...
        if ($modelClass === Comment::class) {
            ...
            $repliesCount = Comment::where('parent_id', $model->getKey())->count();
            $reactService->setCount($modelClass, $model->getKey(), 'replies', $repliesCount);
        }
        ...
        if ($modelClass === User::class) {
            $followersCount = Follow::where('followable_type', $modelClass)
                ...
                ->count();
            $reactService->setCount($modelClass, $model->getKey(), 'followers', $followersCount);

            $followingsCount = Follow::where('follower_id', $model->getKey())->count();
            $reactService->setCount($modelClass, $model->getKey(), 'followings', $followingsCount);
        }
    }
}
