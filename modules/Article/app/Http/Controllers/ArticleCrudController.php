...
class ArticleCrudController extends Controller
{
    ...
    {
        $topCategories = $article->categories->select(['name', 'slug'])->toArray() + $this->feedService->categories();

        $topTags = $article->tags->select(['name', 'slug'])->toArray() + $this->feedService->tags();
